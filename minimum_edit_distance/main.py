...
def min_edit_distance(distance_matrix: Grid, source: str, target: str) -> int:
    ...
    for i in range(1, n + 1):
        for j in range(1, m + 1):
            replace_cost_adjusted_for_same_elements = (
                0 if source[i - 1] == target[j - 1] else REPLACE_COST
            )
...
def main():
    ...
    min_distance = min_edit_distance(
        distance_matrix=distance_matrix, source=source, target=target
    )
